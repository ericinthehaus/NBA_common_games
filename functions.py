...
def common_games(games = games, teamA=teamA, teamB=teamB): 
    ...
    df['record_A'] = df['wins_A'].astype(int).astype(str) + " - " + df['losses_B'].astype(int).astype(str)
    df['record_B'] = df['wins_B'].astype(int).astype(str) + " - " + df['losses_B'].astype(int).astype(str) 

    df = df[(df["num_A"] > 1) & (df["num_B"] > 1)].sort_values(['num_A','num_B'], ascending=False)
